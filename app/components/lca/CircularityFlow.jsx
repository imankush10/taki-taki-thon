...
export function CircularityFlow({ data }) {
    ...
    datasets: [
      {
        ...
        backgroundColor: [
          data.circularityScore >= 70 ? 'rgba(16, 185, 129, 0.8)' : 
          data.circularityScore >= 50 ? 'rgba(245, 158, 11, 0.8)' : 
          'rgba(239, 68, 68, 0.8)',
          'rgba(107, 114, 128, 0.2)'
        ],
        borderColor: [
          data.circularityScore >= 70 ? 'rgba(16, 185, 129, 1)' : 
          data.circularityScore >= 50 ? 'rgba(245, 158, 11, 1)' : 
          'rgba(239, 68, 68, 1)',
  ...
  return (
    <div className="bg-slate-800 dark:bg-slate-900 p-6 rounded-lg shadow-lg border border-slate-600">
      ...
      {/* Material Flow Chart */}
      <div className="mb-8">
        <h4 className="text-md font-medium text-white mb-3">Material Flow Through Lifecycle</h4>
        <div className="h-48">
        ...
        <div className="relative w-48 h-48">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className={`text-xs px-2 py-1 rounded-full mt-1 ${
              data.circularityScore >= 70 ? 'bg-green-900/30 text-green-300' :
              data.circularityScore >= 50 ? 'bg-yellow-900/30 text-yellow-300' :
              'bg-red-900/30 text-red-300'
            }`}>
              {data.circularityScore >= 70 ? 'Excellent' : 
               data.circularityScore >= 50 ? 'Good' : 'Needs Improvement'}
            </span>
          </div>
        </div>
      </div>
